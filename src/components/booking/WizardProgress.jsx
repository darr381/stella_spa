      ...
      {steps.map((step) => {
        ...
        return (
          <div key={step.num} className="flex flex-col items-center gap-3 bg-base-cream px-2">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-serif text-lg transition-all duration-500 ${
              isActive ? 'bg-nature-green text-white shadow-lg scale-110' : 
              isCompleted ? 'bg-lavender text-white' : 'bg-white text-nature-green/40 border border-nature-green/10'
            }`}>
              {step.num}
            </div>
            <span className={`text-[10px] md:text-sm font-sans font-medium uppercase tracking-wider transition-colors duration-500 ${
              isActive ? 'text-nature-green' : isCompleted ? 'text-lavender' : 'text-nature-green/40'
            }`}>
              {step.title}
            </span>
          </div>
